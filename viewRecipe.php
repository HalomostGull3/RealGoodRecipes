...
require_once "Interfaces/vendor/autoload.php";
...
if($recipeDisplay!=null){
    ...
    //add name to pdf
    $html = '<div><h1><b>'.$recipeDisplay["Name"].'</b></h1></div>';

    //add image to pdf
    if ($recipeDisplay["image"]  == null) {
        $html .= '<div class="col 6"><img src="logo.png" ></div>';
    }else{
        $html .= '<div class="col 6"><img src=$recipeDisplay->getImage()></div>';
    }

    //difficulty
    $html .= '<h6><b>Difficulty(1-5):'.$recipeDisplay["Difficulty"].'<p></p></b></h6>';

    //instruction
    $html.= '<div><h6>'.$recipeDisplay["Instructions"].'</h6><p></p></div>';

    //
    $html.='<div><h6>'.$recipeDisplay["Ingredients"].'</h6></div>';

}else{
    $html = "There is no data in this recipe,try it again";
}
...
<div class="row">
    ...
    <div class="col 12">
        ...
        <div class = "box">
            <button onclick = "printPDF()">Download it!</button>
            <script>
                <?php
                $mpdf -> Output('recipe.pdf',"D");
                ?>
            </script>
        </div>
    </div>
